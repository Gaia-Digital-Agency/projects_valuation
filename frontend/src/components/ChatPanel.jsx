...
export default function ChatPanel({ company }) {
  ...
  useEffect(() => {
    setMessages([{ role: 'assistant', content: `Ask me anything about **${company}**'s valuation, risks, or specific line items.` }])
  }, [company])
  ...
  const send = async (text) => {
    ...
    try {
      const r = await api.chat(company, q, next.slice(0, -1))
      setMessages((m) => [...m, { role: 'assistant', content: r.reply }])
    } catch (e) {
      setMessages((m) => [...m, { role: 'assistant', content: `_Error: ${e.message}_` }])
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <section className="panel min-h-[420px]">
      <div className="panel-header">
        ...
        <span className="text-xs text-slate-500">claude-haiku-4-5</span>
      </div>
      ...
      <div className="px-4 py-2 border-t border-ink-700/40">
        <div className="flex flex-wrap gap-1 mb-2">
          {SUGGESTIONS.map((s) => (
            <button key={s} onClick={() => send(s)}
              className="text-[11px] px-2 py-1 rounded border border-ink-700 text-slate-300 hover:bg-ink-700/50">{s}</button>
          ))}
        </div>
        <form onSubmit={(e) => { e.preventDefault(); send() }} className="flex gap-2">
          ...
          <button type="submit" disabled={sending || !input.trim()}
            className="bg-accent-600 hover:bg-accent-500 disabled:opacity-40 text-white text-sm px-4 rounded-md">Send</button>
        </form>
      </div>
    </section>
  )
